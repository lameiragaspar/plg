...
**2. Cadastrar o utilizador administrador.** Duas opções:

- **Opção A — Dashboard (recomendada):**
  *Authentication → Users → Add user*. Preencher **Email** e **Password** e
  marcar **Auto Confirm User** (para não exigir confirmação por email).
  Clicar *Create user*.

- **Opção B — SQL (avançada):** caso prefira via SQL Editor, usar a função
  oficial do Supabase (faz o *hash* da senha por baixo):

  ```sql
  -- Substituir o email e a palavra-passe.
  select auth.admin_create_user(
    '{"email":"admin@exemplo.com","password":"UMA_SENHA_FORTE","email_confirm":true}'::jsonb
  );
  ```

  > Nunca inserir directamente em `auth.users` à mão — a senha tem de passar
  > pelo *hashing* do Supabase. Use sempre o dashboard ou a API/função `admin`.
